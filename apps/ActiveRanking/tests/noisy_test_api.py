...
def test_api(assert_200=True, num_arms=100, num_clients=20, delta=0.05,
             total_pulls_per_client=1000, num_experiments=1, noiselevel=10):
    ...
    pool_args = [(numpy.random.choice(exp_info)['exp_uid'],
                  ...
                  true_means,assert_200,noiselevel) for participant_uid in participants]
...
def compare(i, j, noiselevel):
    lamb = 1./noiselevel
    np.random.seed()
    if np.random.random() < 1./(1+np.exp(-lamb*(i-j))):
        return i
    else:
        return j

def simulate_one_client(input_args):
    exp_uid,participant_uid,total_pulls,true_means,assert_200,noiselevel = input_args

    getQuery_times = []
    processAnswer_times = []
    ts = test_utils.response_delay(mean=5*np.random.rand(), std=.1)
    for t in range(total_pulls):
        ...
        target_winner = compare(left['target_id'], right['target_id'], noiselevel)
